...
function drawHexPixel(cx, cy, col) {
  ...
  for (let x = cx - hexSize; x <= cx + hexSize; x += pixelSize - 0.8) {
    for (let y = cy - hexSize; y <= cy + hexSize; y += pixelSize - 0.8) {
      if (pointInPolygon(x, y, verts))
        rect(x, y, pixelSize + 1.0, pixelSize + 0.8);
    }
  }
}
...
function drawUI() {
  ...
  if (selectedClubIndex != null) {
    ...
    text(`Selected: ${c.name}  → Move: ${selectedMoveCount}`, 10, 30);
  } else {
    text("Select a club (left) and optional modifiers (right)", 10, 30);
  }
